 ...
 */
/*
int u_putchar(char c, FILE *unused)
{
...
int ee24xx_read_bytes(uint16_t eeaddr, int len, uint8_t *buf)
{
  ...
  switch ((twst = TW_STATUS))
    ...
    case TW_START:  break;
    case TW_MT_ARB_LOST:  goto begin;	/* Note [9] */
    default:  return -1;		/* error: not in start condition */
  ...
  switch ((twst = TW_STATUS))
    ...
    case TW_MT_SLA_NACK: goto restart;	/* nack during select: device busy writing */
				/* Note [11] */
    case TW_MT_ARB_LOST: goto begin;	/* re-arbitrate */
    default: 			 goto error;		/* must send stop condition */
    }
...
//************************************************************************************************
//FILE mystdout = FDEV_SETUP_STREAM(u_putchar, NULL, _FDEV_SETUP_WRITE);
//************************************************************************************************
void test(void)
{
  ...
  for (a = 0; a < 256;)
    {
      ...
      if (rv <= 0) 	error();
      if (rv < 16) 	printf("warning: short read %d\r\n", rv);
      a += rv;
      for (x = 0; x < rv; x++) printf("%02x\r\n", b[x]);
    }

  rv = EE_WRITE(55, message);
  if (rv < 0)    error();
